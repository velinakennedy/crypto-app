...
const MarketTable = () => {
  ...
  const { currentData, isSuccess } = useGetCoinTableQuery(
    { currency, page: queryInfo.page },
    { skip: queryInfo.skip }
  );
  ...
  const renderCell = (item: MarketFormattedData, columnKey: Key) => {
    ...
    switch (columnKey) {
      ...
      case "last7Days":
        return (
          <div className="flex justify-center">
            {hasArray && <MarketChart data={cellValue as [number, number[]]} />}
          </div>
        );
      default:
  ...
  useEffect(() => {
    if (currentData && isSuccess) {
      setFormattedData(
        formattedData
          ? [...formattedData, ...formatMarketTableData(currentData)]
          : formatMarketTableData(currentData)
      );
                  ...
                  <TableColumn key={header.key} allowsSorting={header.sortable}>
                    <div
                      className={`flex gap-2 justify-center items-center ${
                        header.sortable ? "hover:cursor-pointer" : ""
                      }`}
                    >
                      {header.title}
