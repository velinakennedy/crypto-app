                ...
                {formatter && item?.value !== undefined && item.name ? (
                  ...
                  <>
                    ...
                    <div className={cn("flex flex-1 justify-between leading-none", nestLabel ? "items-end" : "items-center")}>
                      ...
                      {item.value && (
                        <span className="font-medium font-mono text-slate-950 dark:text-slate-50 tabular-nums">{item.value.toLocaleString()}</span>
                      )}
                    </div>
                  </>
                )}
    ...
    <div ref={ref} className={cn("flex items-center justify-center gap-4", verticalAlign === "top" ? "pb-3" : "pt-3", className)}>
      {payload.map((item) => {
        ...
        return (
          <div
            key={item.value}
            className={cn("flex items-center gap-1.5 [&>svg]:h-3 [&>svg]:w-3 [&>svg]:text-slate-500 dark:[&>svg]:text-slate-400")}
          >
